...
pub fn render(state: &mut State, ui: &mut Ui) {
    render_board(&mut state.board, ui, vec2(400.0, 400.0), state.style);
}

fn render_board(board: &mut crate::Board, ui: &mut Ui, size: egui::Vec2, style: BoardStyle) {
    ...
    if response.clicked() {
        if let Some(p) = ui.input(|i| i.pointer.interact_pos()) {
            let (x, y) = (
                ((p.x - inner_rect.min.x) / spacing.x).round() as usize,
                ((p.y - inner_rect.min.y) / spacing.y).round() as usize,
            );

            board.set(x, y, crate::Stone::Black);
        }
    }
}
